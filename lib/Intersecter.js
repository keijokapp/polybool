...
import { addPoint, pointLabel, segLabel } from '../test.js';
...
export function sweep(segments, selfIntersection) {
	...
	while (events.length) {
		...
		if (ev.isStart) {
			...
			assert(!isEqualVec2(ev.seg.data.p0, ev.seg.data.p1));
...
function checkIntersection(events, ev1, ev2) {
	...
	const ba1x = r.minus(b1[0], a1[0]);
	const ba1y = r.minus(b1[1], a1[1]);

	if (r.eq(someFuckingProduct(ba0x, ba0y, dax, day), 0)
		&& r.eq(someFuckingProduct(ba1x, ba1y, dax, day), 0)) {
		// lines are coincident
		const a1b1 = compareVec2(a0, b0);
		const a1b2 = compareVec2(a0, b1);
		...
		// otherwise, parallel or intersecting

		const dbx = r.minus(b1[0], b0[0]);
		const dby = r.minus(b1[1], b0[1]);

		const axb = someFuckingProduct(dax, day, dbx, dby);

		if (r.eq(axb, 0)) {
			// lines are parallel, with no intersections
			return;
		}

		const t = r.div(someFuckingProduct(ba0x, ba0y, dbx, dby), axb);
		const u = r.div(someFuckingProduct(ba0x, ba0y, dax, day), axb);
		...
		if (t0 >= 0 && t1 <= 0 && u0 >= 0 && u1 <= 0) {
			...
			/** @type {Vec2} */
			const pb = [
				r.plus(b0[0], r.times(dbx, u)),
				r.plus(b0[1], r.times(dby, u)),
			];
...
function compareSegments(seg1, seg2) {
	const oldResult = r.cmp(orient2d(seg1.p0, seg2.p1, seg2.p0), 0)
		|| r.cmp(orient2d(seg1.p1, seg2.p1, seg2.p0), 0);

	if (r.lt(seg2.p0[0], seg1.p0[0])) {
		return oldResult;
	}

	const newResult = r.cmp(orient2d(seg2.p0, seg1.p0, seg1.p1), 0)
		|| r.cmp(orient2d(seg2.p1, seg1.p0, seg1.p1), 0);
...
function orient2d(p1, p2, p3) {
	const dx1 = r.minus(p1[0], p2[0]);
	const dy1 = r.minus(p1[1], p2[1]);
	const dx2 = r.minus(p2[0], p3[0]);
	const dy2 = r.minus(p2[1], p3[1]);

	return r.cmp(someFuckingProduct(dx1, dy1, dx2, dy2), 0);
}
 ...
 * @returns {Rational}
 */
function someFuckingProduct(dx1, dy1, dx2, dy2) {
