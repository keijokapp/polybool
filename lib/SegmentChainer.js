 ...
 * @returns {Vec2[][]}
 */
export function polygon(segments, epsilon = Number.EPSILON) {
	/** @type {Vec2[][]} */
	const chains = [];
	/** @type {Vec2[][]} */
	const regions = [];
...
function isCollinear(p1, p2, p3, epsilon) {
	...
	const orientation = orient2d(p1, p2, p3);
	const baseLengthSq = (p3[0] - p1[0]) ** 2 + (p3[1] - p1[1]) ** 2;

	return baseLengthSq === 0
		? orientation === 0
		: orientation ** 2 < epsilon ** 2 * baseLengthSq;
}

/**
 * Direction of p1 related to line p2->p3.
 *   >0 - p1 is CCW (left) of the directed line p2->p3
 *   <0 - p1 is CW (right) of the directed line p2->p3
